...
export function useGetEmojiAnimationFrames() {
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(() => {
      if (emo.isRandom) {
        // 随机播放
        const exclusiveEmo = emoInState
          .map((item: any, index: number) => index)
          .filter((item: number) => item !== animationFrames);
        const random =
          exclusiveEmo[Math.floor(Math.random() * exclusiveEmo.length)];
  ...
  const emoInfo = useMemo(
    () => emoInState[animationFrames](),
    [emoInState, animationFrames],
  );
